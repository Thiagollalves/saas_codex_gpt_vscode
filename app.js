...
const updateConnectionStatus = (type, message) => {
  connectionStatuses.forEach((status) => {
    if (status.dataset.status === type) {
...
connectButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (type === "whatsapp") {
      const qrBox = document.querySelector('[data-qr="whatsapp"]');
      qrBox.classList.toggle("is-hidden");
      const isVisible = !qrBox.classList.contains("is-hidden");
      button.textContent = isVisible ? "Fechar QR Code" : "Gerar QR Code";
      updateConnectionStatus(
        "whatsapp",
        isVisible ? "QR Code gerado. Escaneie para conectar." : "Não conectado."
      );
      return;
    }

    const providerLabel = type === "facebook" ? "Facebook" : "Instagram";
    button.textContent = "Conectado";
    button.classList.add("secondary");
    button.disabled = true;
    updateConnectionStatus(type, `${providerLabel} conectado com sucesso.`);
  });
});
