...
subtabButtons.forEach((button) => {
  button.addEventListener("click", () => {
    subtabButtons.forEach((item) => item.classList.remove("is-active"));
    subtabPanels.forEach((panel) => panel.classList.remove("is-active"));
...
integrationButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (card.dataset.integration === "whatsapp") {
      if (qrBox) qrBox.classList.toggle("is-hidden");
      const isVisible = qrBox && !qrBox.classList.contains("is-hidden");
      if (status) {
        status.textContent = isVisible
          ? "QR Code gerado. Aguardando leitura."
          : "QR Code não gerado.";
      }
      button.textContent = isVisible ? "Ocultar QR Code" : "Gerar QR Code";
      return;
    }

    const isConnected = button.classList.toggle("is-connected");
    button.textContent = isConnected ? "Desconectar" : "Conectar";
    if (status) {
