...
const setQrStatus = (text, isLive) => {
  if (!qrStatus) return;
  qrStatus.textContent = text;
  qrStatus.classList.toggle("is-live", Boolean(isLive));
};
...
const buildFallbackQrUrl = (token) => {
  const safeToken = token?.trim() || "kamba-whatsapp-session";
  return `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(
    `https://web.whatsapp.com/qr/${safeToken}`
  )}`;
};

const requestWhatsappQr = async (token) => {
  ...
  if (!response.ok) {
    throw new Error(`Falha ao solicitar QR: ${response.status}`);
  }
...
const generateQrCode = async (token) => {
  if (!qrImage) return;
  setQrStatus("Gerando QR...", false);
  try {
    const data = await requestWhatsappQr(token);
    qrImage.src = data.qr_url;
    setQrStatus("QR pronto para leitura.", true);
  } catch (error) {
    qrImage.src = buildFallbackQrUrl(token);
    setQrStatus("QR local gerado (modo demo).", false);
    showToast("Backend indisponível, exibindo QR de demonstração.");
  }
};
