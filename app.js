...
const integrationButtons = document.querySelectorAll("[data-connect]");
const qrCode = document.getElementById("qr-code");
const addAgentForm = document.getElementById("add-agent");
...
const setIntegrationStatus = (key, text, isConnected = false) => {
...
integrationButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (key === "whatsapp") {
      ...
      setIntegrationStatus("whatsapp", "QR gerado - aguardando leitura");
      return;
    }
    setIntegrationStatus(key, "Conectado", true);
  });
});

addAgentForm.addEventListener("submit", (event) => {
  event.preventDefault();
  const data = new FormData(addAgentForm);
  const agentName = data.get("agentName");
  if (!agentName) return;
  const li = document.createElement("li");
  const dot = document.createElement("span");
  dot.classList.add("status-dot", "online");
  const nameNode = document.createTextNode(agentName.toString());
  const meta = document.createElement("span");
  meta.classList.add("muted");
  meta.textContent = "0 chats";
  li.appendChild(dot);
