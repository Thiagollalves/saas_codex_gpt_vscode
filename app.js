...
const metaApiForm = document.getElementById("meta-api-form");
...
const savedApiConfig = localStorage.getItem("metaApiConfig");
if (savedApiConfig) {
  apiStatus.textContent = "Status: conectado (token salvo localmente)";
...
metaApiForm.addEventListener("submit", (event) => {
  event.preventDefault();
  const data = new FormData(metaApiForm);
  const endpoint = data.get("endpoint");
  const phoneId = data.get("phoneId");
  const token = data.get("token");

  if (endpoint && phoneId && token) {
    apiStatus.textContent = "Status: conectado (token salvo localmente)";
    apiStatus.style.color = "#10b981";
    localStorage.setItem(
      "metaApiConfig",
      JSON.stringify({ endpoint, phoneId })
    );
  } else {
    apiStatus.textContent = "Status: preencha todos os campos";
