...
const qrSimulate = document.getElementById("qr-simulate");
...
const actionHandlers = {
  ...
  "refresh-campaigns": () => {
    ...
    showToast(`Campanhas atualizadas (${rows.length} totais).`);
  },
  "export-dashboard": () => {
  ...
  "funnels-refresh": () => {
    ...
    showToast(`Funis atualizados (${cards.length}).`);
  },
  "funnels-history": () => {
    const history = [
      `Envio 01 • ${toTimeStamp()} • 420 leads`,
      `Envio 02 • ${toTimeStamp()} • 380 leads`,
      `Envio 03 • ${toTimeStamp()} • 512 leads`,
    ];
    openDrawer({
      title: "Historico de envios",
      fields: history.map((item) => ({ type: "info", value: item })),
      confirmText: "Fechar",
      onSubmit: () => {},
    });
  },
  ...
  "save-profile": () => {
    ...
    updateStoredArray("profile", () => [{ name, email, at: toTimeStamp() }]);
    showToast("Perfil atualizado.");
  },
  "update-preferences": () => {
    const selects = document.querySelectorAll("#settings select");
    const preferences = Array.from(selects).map((select) => ({
      name: select.closest("label")?.textContent?.trim() || "Preferencia",
      value: select.value,
    }));
    updateStoredArray("preferences", () => preferences);
...
const updateQrStatus = (status, countdown) => {
  if (qrStatus) qrStatus.textContent = status;
  if (qrTimer) qrTimer.textContent = countdown;
};
...
const startQrSession = () => {
  ...
  updateQrStatus("Aguardando leitura", formatCountdown(qrCountdown));
  qrInterval = setInterval(() => {
    qrCountdown -= 1;
    if (qrCountdown <= 0) {
      stopQrTimer();
      updateQrStatus("QR expirado. Atualize para continuar.", "00:00");
      showToast("QR expirado. Gere um novo codigo.");
      return;
    }
    updateQrStatus("Aguardando leitura", formatCountdown(qrCountdown));
  }, 1000);
...
const markQrConnected = () => {
  qrConnected = true;
  stopQrTimer();
  updateQrStatus("Conectado", "00:00");
  const card = document.querySelector('.integration-card[data-channel="whatsapp"]');
  if (card) {
    const badge = card.querySelector(".status-badge");
    const button = card.querySelector(".integration-action");
    badge.classList.remove("offline");
    badge.classList.add("online");
    badge.textContent = "Conectado";
    if (button) button.textContent = "Conectado";
  }
...
integrationActions.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (channel === "whatsapp") {
      if (qrConnected) {
        showToast("WhatsApp ja conectado.");
        return;
      }
      startQrSession();
      button.textContent = "Aguardando leitura";
      return;
    }
...
if (qrSimulate) {
  qrSimulate.addEventListener("click", () => {
    markQrConnected();
  });
}
