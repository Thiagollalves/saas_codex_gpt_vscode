...
export default function HomePage() {
  ...
  const [whatsappQrMode, setWhatsappQrMode] = useState<"live" | "fallback">("live");
  ...
  useEffect(() => {
    ...
    fetchJson<{ status: string; qr: string; mode?: "fallback" }>("/api/whatsapp/qr", {
      status: "DISCONNECTED",
      qr: "",
      mode: "fallback"
    }).then((data) => {
      ...
      setWhatsappQrMode(data.mode === "fallback" ? "fallback" : "live");
    });
  }, []);
            ...
            <Card className="lg:col-span-2">
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[220px_1fr]">
                <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4 text-center text-xs text-zinc-400">
                  ...
                  {whatsappQrMode === "fallback" ? (
                    <span className="mt-2 text-[10px] text-amber-300">
                      Modo offline: QR de demonstração
                    </span>
                  ) : null}
                </div>
