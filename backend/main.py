...
from backend.services.whatsapp_qr import build_qr_url
...
@app.post("/api/whatsapp/qr", response_model=WhatsAppQrResponse)
async def whatsapp_qr(request: WhatsAppQrRequest) -> WhatsAppQrResponse:
    token = request.token or "kamba-session"
    qr_url = build_qr_url(token)
    return WhatsAppQrResponse(qr_url=qr_url, session=token, provider=settings.whatsapp_qr_service_url)
